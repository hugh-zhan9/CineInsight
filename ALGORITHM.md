...
```text
实际播放分数 = 播放分数 × 2 ^ (-距上次播放天数 / 半衰期天数)
```

半衰期默认 90 天。每经过一个半衰期，历史次数的贡献减半，使久未观看的视频自然回到随机池。没有上次播放时间的记录保持原分数；半衰期设为 `0` 时关闭衰减，计算结果与旧算法完全一致。
...
半衰期配置同样在下次随机播放时立即生效；它只改变选中概率，不改变播放成功后才更新统计的可靠性语义。
