...
// 扫描后自动任务的五个开关必须真的存得下来。
//
// 这条以前是缺的：前端一直把这些字段发过来，UpdateSettings 却没有赋值，
...
func TestUpdateSettingsPersistsAutomationToggles(t *testing.T) {
	...
	if !saved.AutoTechnicalBackfill || !saved.AutoPerceptualHash || !saved.AutoCleanupAnalysis ||
		!saved.AutoImageEXIFBackfill || !saved.AutoImagePerceptualHash {
		t.Fatalf("五个自动开关都应被保存为开(%s): technical=%v phash=%v cleanup=%v exif=%v image_phash=%v",
			dbtest.Backend(), saved.AutoTechnicalBackfill, saved.AutoPerceptualHash,
			saved.AutoCleanupAnalysis, saved.AutoImageEXIFBackfill, saved.AutoImagePerceptualHash)
	}
	...
	if saved.AutoTechnicalBackfill || saved.AutoPerceptualHash || saved.AutoCleanupAnalysis ||
		saved.AutoImageEXIFBackfill || saved.AutoImagePerceptualHash {
		t.Fatalf("五个自动开关都应被保存为关(%s): technical=%v phash=%v cleanup=%v exif=%v image_phash=%v",
			dbtest.Backend(), saved.AutoTechnicalBackfill, saved.AutoPerceptualHash,
			saved.AutoCleanupAnalysis, saved.AutoImageEXIFBackfill, saved.AutoImagePerceptualHash)
	}
}
