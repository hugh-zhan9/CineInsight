...
| 版本号 | 修订内容 | 修订时间 | 修订人 |
|---|---|---|---|
| V1.0.17 | 用户授权降低截取片段误判，并明确保留调色片段识别；D-027 改为有效哈希证据与本地归一化结构复核，保留现有存储和忽略合同 | 2026-09-22 | Codex |
...
<a id="D-027"></a>
**D-027｜匹配规则（AC-17；behavior contract，2026-09-22 按用户误判反馈修订）**：用户授权优化“一眼不相似的画面被判为截取片段”；保留 2 秒序列、时长比例、人工审阅、忽略版本规则与其余清理类别。

- 哈希初筛仍归 `clip_match.go`：B 至少 10 秒、长度 ≤ A 的 90%；单帧汉明距离 ≤8，整段有效命中率 ≥80%。1 位数少于 8 或多于 56 的指纹不提供匹配证据（旧数据无需重算）。粗筛均匀检查最多 16 帧，至少 75% 命中；全段四个时间区间各至少一半命中，并至少有 3 种彼此距离 >8 的已匹配指纹，避免纯色、近静态、重复片头撑高分数。低信息帧保留时间位置，仍计入分母。
- `CleanupService` 内增加本地画面结构复核：只对可能更新该片段最佳候选的配对执行，在全段已命中位置均匀取最多 5 处（至少 3 处），按统一偏移读取双方对应 32×32 RGB 帧。RGB 转亮度后比较去均值、方差归一化的结构：全图相关性 ≥0.90，至少 8 个有结构的 4×4 块，且至少 80% 的有结构块相关性 ≥0.80 才命中（亮度标准差 <3 的块视为平坦；两侧都平坦不计证据，单侧平坦算失败）；至少 80% 抽样处通过才输出候选。命中率字段仍是哈希命中比例，不代表准确率。
- 所有复核帧只驻留当前调用内存，不外发、不落表；复用既有 ffmpeg 查找能力。现有 `ffmpegPerceptualFrameRunner` 只有 9×8 灰度且允许回退到其他时间，`sampleAITaggingFrames` 为大图和 AI data URL，均不适合精确时点的本地颜色复核；专用小帧抽取归 `clip_verify.go`，不新建服务或任务。每对顺序抽样，最多 10 次单帧读取，30 秒总期限，并继承应用关闭上下文。抽样前后核对双方 size/mtime 与原序列一致。
- 失败边界：复核不通过只排除该配对，其他完整片仍可竞争；2026-09-24 用户裁决：读取、解码、源变化或配对超时只跳过当前配对，继续其余候选和旧五类结果；`skipped_clip_verification` 记录本轮跳过配对数并在清理面板提示，重新分析时重试。应用父上下文取消/超时仍终止整轮，数据库错误仍向上传递。禁止把未经复核的结果回给用户。错误只含视频 ID 与阶段，不附源路径/ffmpeg 原始输出。数据库查询、持久忽略与删除语义不变。
- 验证：纯色碰撞、近静态、共用片头但后半不同、灰度指纹相近但空间结构不同均应拒绝；真实截取、重编码缩放和调色仍应识别。同步验证忽略与旧五类快照。当前没有用户样片，合成及真实 ffmpeg 夹具不能代表用户片库的实际误判率。用户进一步明确“调色后的片段不该漏掉”：颜色差异不作为否决条件，允许亮度、对比度、色调和饱和度改变，以真实 ffmpeg 调色夹具验证。缺乏结构的静态素材仍不出候选；局部重绘或改变主体结构不属于普通调色；不承诺倍速、镜像、空间裁剪。

此调整是已授权的局部算法实现选择；阈值以回归夹具验证，不新增产品开关、表或公共方法。Support lenses: `go-style`。
...
1. 回填任务产出序列。
2. 清理分析末尾：取所有有效序列 → 排除忽略与重复对 → 时长/哈希证据预筛 → 全段验证 → 本地结构复核 → 保留通过复核的最佳配对。
...
| 黑场/静态画面导致误配 | 低信息过滤、时序证据与结构复核 | 证据不足不出候选 | fixture |
...
- `plan2exec` 可以决定：切片切分与顺序（须满足「代码健康拆分先于触碰 `VideoListPage.vue` / `app.go` / `video_service.go` 的功能切片」与「人脸切片串行排在最后」——共享文件决定了写入不可能不相交）；文件与函数命名；测试 fixture 构造；阈值在本文给出的初值上按 fixture 微调（D-017 0.55/0.6、D-027 当前阈值见 §4.6.2，以误判与调色 fixture 共同验证）；命令面板动作清单的具体条目；前端子组件的粒度。
...
| TC-08 | AC-17, AC-18 | D-026..D-028 | 合成序列、结构碰撞、真实 ffmpeg 截取/缩放/重编码/调色夹具；忽略与旧类别快照 | automation | 缺用户误判样片，实际片库误判率待样片验证 |
...
| D-026 | AC-17 | data + operations | 2 s 间隔 uint64 序列；单趟 ffmpeg；单 worker；三点 pHash 不动 | — | 基准 ≤ 3 min/2h；pHash 的 stale 计数口径已于 2026-09-20 被 [清理候选检测修复 D-CD04](../2026-09-20-cleanup-detection-fix/概要设计.md#4-决定) 改为与帧哈希同口径（未回填 + 失效），算法本身仍不动 |
| [D-027](#D-027) | AC-17 | behavior | 哈希证据筛选与本地画面结构复核 | 静态证据不足不出候选；不识别倍速/镜像/裁剪 | 合成及真实 ffmpeg fixture |
