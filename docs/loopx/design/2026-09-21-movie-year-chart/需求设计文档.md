...
### 6.2 海报代理路由（[D-MC09](概要设计.md#D-MC09)）
...
| TC-11 海报代理的地址来源与域名白名单 | 安全边界 | `TestDoubanChartPosterRouteRejectsAddressesOutsideWhitelist`（10 个地址） | **通过**。含 `evildoubanio.com` 近似命中；另有逐跳重定向校验 |
