...
#### 4.6.6 2026-08-08 裁决修订：AI 产出由「描述」改为「标签」
...
| D-009 | AC-5 | data/interface | **2026-08-08 撤销**：改为产出标签候选并走与视频一致的候选/审阅流程（见 4.6.6）。原契约为描述单表 + 中文 80–200 字禁标签 | 无 run 历史表；不进 AI 质量评估 | 状态机可续跑、脱敏落库 |
| D-010 | AC-6 | data | 共享 SemanticIndexProfile；镜像三件套表 + image_semantic_vectors（HNSW cosine）；索引文本=标题+标签（**2026-08-08 修订**：原为标题+标签+描述且无描述 Skipped，见 4.6.6） | 图片不触发 rebuild | 全局语义任务互斥 |
