...
- **AC-5 已失效**：intake 原文写的是「结果为自然语言描述（非标签候选）」，已被 4.6.6 整条推翻。
  以 4.6.2 的重写版为准：可对图片触发 AI 打标，结果为标签候选，经人工审阅接受后写入 `image_tags`，可重跑。
  intake 原文保持不动（它是历史记录），不要按它回退实现。
...
| 清理审阅 | 独立整页审阅视图 | ImageService 分析方法 | size+采样MD5 精确分组；dHash 汉明近似分组 | stale 哈希跳过并计数 | 按目录分组（精确重复默认勾选多余副本，近似重复不勾） |
...
- 页面契约（**D-015**，behavior contract）：`PhotoLibraryPage.vue` 新组件；CSS Grid `repeat(auto-fill, minmax(180px, 1fr))`，卡片 `aspect-ratio: 1` + `object-fit: cover`，`<img loading="lazy" :src="/preview/image-thumbnail/{id}">` + `@error` 占位降级；IntersectionObserver 哨兵（root=`.main-view`，rootMargin 320px）+ 手动"加载更多"兜底；每页 60 张。竞态防护用 token（照抄 EntityLibraryPage 的 Symbol token 模式）。

> **2026-08-07 修订（用户裁决推翻本段部分内容，见 P-013）**：原文「首期不做网格虚拟化」已作废——网格虚拟化与时间线分组均已交付。随之被取代的实现细节：列宽由 `repeat(auto-fill, minmax(180px,1fr))` 改为按行显式 `repeat(var(--photo-columns), …)`（JS 与 CSS 必须对齐列数）；IntersectionObserver 哨兵改为按滚动位置在距底 400px 预取（虚拟化后哨兵不再可靠常驻 DOM）。`loading="lazy"`、`@error` 占位、每页 60、Symbol token 竞态防护均不变。窗口计算见 `frontend/src/utils/photoGrid.js`（前缀和二维分组模型，不复用一维的 `virtualList.js`）。
...
> **2026-08-18 已落地。** 本节描述的是当前实现。原「AI 描述」契约（D-009）已于 2026-08-08 撤销、
> 2026-08-18 完成改造并删除全部遗留数据与代码，历史结论保留在 4.6.6，不要按它回退。
> 执行记录见 `docs/loopx/plans/2026-08-18-image-ai-tags.md`。

#### 4.6.2 方案设计

（**D-009 重立**，data + interface contract）

- 存储：三张图片侧表镜像视频侧形态，`video_id` 换 `image_id`（外键 `images` + CASCADE）：
  `image_ai_tag_candidates`（候选，状态 pending/approved/rejected/superseded，无 `run_id`）、
  `image_ai_tag_approval_records`（`(image_id, tag_id)` 唯一、`candidate_id` 唯一，是「哪些
  `image_tags` 关联由 AI 接受产生」的唯一依据）、`image_ai_tagging_states`（`image_id` 唯一，
  承载 status/skip_reason/evidence_fingerprint/attempt_count/last_error/last_processed_at）。
  不建 run 历史表（AI 质量评估不覆盖图片）。视频侧五张 `ai_tag*` 表一字未动。
  另有 `(image_id, normalized_name) WHERE status='pending'` 部分唯一索引，由
  `database.EnsureImageAITaggingIndexes` 建（GORM 标签表达不了带 WHERE 的唯一索引）。
- 请求：复用 `SettingsAITaggingConfigProvider` 配置；单图一次请求（**单轮，不是视频那套多轮
  agent**——抽帧/字幕/同源对静图都不适用），messages = system 提示 + user content
  [提示文本, `{"type":"image_url","image_url":{"url":"data:image/jpeg;base64,<缩略图管线产物>"}}`]；
  `temperature 0.1`、超时 5 分钟、无客户端重试（重试语义由 `attempt_count` 承担）。
- 提示词契约：要求模型只从闭合标签词表里选标签并输出 JSON
  `{"suggestions":[{"label","confidence","reasoning"}]}`；复用视频侧的
  `formatClosedTagLibraryForPrompt` 呈现词表与 `parseAITagSuggestions` 解析响应。
  词表口径是 `is_system AND is_active`（**不是** `TagService.GetAITagLibrary` 的只过滤
  `is_system`，那会把停用标签一起喂给模型）。
- 落库：low 置信度丢弃；库外标签一律丢弃且绝不新建 tag；重打前把该图旧的待审候选整体置
  `superseded`；用户拒绝过的标签不再重复推送。
- 任务：批量三件套 `StartImageAITagging / GetImageAITaggingStatus / CancelImageAITagging`
  （单 worker，可取消、进度事件 `image-ai-tagging-progress`）；单张 `RetagImage(imageID)`
  同步执行并绕过指纹判定。启动时 `processing` 复位为 `failed/interrupted`。
- **目标集只排除 `processing`**，「要不要再发一次 AI」由证据指纹判定而非状态过滤。
  视频侧 `findUntaggedVideos` 把 completed 与 skipped 一起排除，代价是标签库扩充后已打标的
  媒体永远不会按新词表重评；图片侧不复制这个行为。
- 证据指纹 = 图片身份（id/path/name/size）+ 提示词里真正出现的词表条目（namespace+name）。
  **不含** `PerceptualHash`/`HashSourceModTimeNS`（解析缩略图会回填它们，算进去等于把自身副作用
  算进指纹），也**不用** `tagLibraryHash`（含 `updated_at`，改个标签颜色就会让整库重发一遍）。
- 审阅：`ListImageAITagCandidates / ApproveImageAITagCandidate / RejectImageAITagCandidate /
  RejectImageAITagCandidatesByImage / GetImageAITaggingSummary`。接受的事务语义与视频侧
  `ApproveCandidate` 逐条对齐；「已有手工标签」的判定必须**减去审批记录**
  （`hasManualOfficialImageTags`），用 `hasNonAutomaticTags` 会把 AI 接受过的标签误判为手工，
  让图片在接受一个候选后永久停在 skipped。
- 输入取材：缩略图管线产物，外发前必须过 `StripJPEGMetadataForUpload`；剥不掉就一个字节都不发
  （fail-closed）。缩略图不可得（非 darwin 的 HEIC/RAW、损坏文件）→ 记
  `failed/decode_unsupported`，批量继续。`is_stale` 的图片不进目标集。

#### 4.6.4 边界条件

| 场景 | 处理方式 | 用户/调用方感知 | 监控/告警 |
|---|---|---|---|
| AI 未配置 | Start/Retag 返回明确错误 | 设置引导提示 | 无 |
| 标签库为空 | 记 `skipped/empty_tag_library`，不发请求 | 状态面板可见跳过数 | log |
| 图片已有手工标签 | 记 `skipped/already_tagged`，不发请求 | 同上 | log |
| 证据指纹未变且上次 completed | 跳过，不发请求 | 同上 | 无 |
| 单图调用失败/超时 | `failed` + 有界 last_error，批量继续；下轮仍会重试 | 状态面板可见失败数 | log |
| 剥元数据失败 | `failed/metadata_strip_failed`，**不外发** | 同上 | log |
| 取消 | context 取消，当前图回退 pending | 任务停止 | 无 |
| 中途崩溃 | processing→failed/interrupted | 可重跑 | log |
| 重复 Start | 运行中拒绝二次启动 | "任务运行中" | 无 |
| 批量与单张重跑撞同一张 | 共用认领表，后到者跳过 | 计入跳过 | 无 |
| 图片在任务中被删除 | 软删检查跳过；硬删由 CASCADE 清理 | 无 | 无 |

#### 4.6.6 2026-08-08 裁决修订：AI 产出由「描述」改为「标签」（**2026-08-18 已实施**）

用户实际使用后判断图片 AI 描述**准确度不足**，决定改造为与视频一致的标签模式。
本节推翻 D-009 与 D-010 的相关条款，后续工作以本节为准：

| 原契约 | 修订后 |
|---|---|
| D-009：图片 AI 产出中文 80–200 字描述，禁标签；单表 `image_ai_descriptions` | 图片 AI 产出**标签候选**，走与视频一致的「候选 → 人工审阅 → 接受/拒绝」流程；标签词表与视频共享 `tags` 表 |
| D-010：索引文本 = 标题 + 标签 + 描述；**无 completed 描述的图片不进索引（Skipped）** | 索引文本 = 标题 + 标签；不再依赖描述，无描述不再是跳过理由 |
| —— | 已生成的图片描述与其语义向量：**按用户明确要求删除**（一次性、不可逆，须单独确认后执行） |

明确接受的代价：图片语义检索的信息量下降。原本描述是索引文本里唯一有实质信息的部分，
改为仅「标题 + 标签」后，语义检索效果会明显弱于改造前，接近关键词匹配。用户已知悉并接受。

保持不变：视频 AI 打标全链路；AI 数据外发边界（仅降采样帧 + 提示词）；
`ai_tagging_*` 配置只读复用。
...
- 任务形态：`StartImageCleanupAnalysis() / GetImageCleanupStatus()` 异步 + 进度事件（镜像 CleanupService）；删除走 `BatchDeleteImages(ids, true)` → 回收站；忽略走 `DismissImageNearDuplicateGroup(ids)`（成对写低/高 ID）。删除/恢复后 `InvalidateAnalysis` 标记结果过期（保留结果供继续审阅，不再丢弃）。UI 为图片库内的整页审阅视图。

默认勾选规则于 2026-08-08 修订（原契约为"候选一律不自动勾选"，实现期演化成"只勾同目录副本"，用户反馈该规则不可预期）：**精确重复**（size + 采样哈希一致，内容即同一份）除保留项外全部默认勾选；**近似重复**（不同尺寸/压缩，属判断题）一律不勾。界面提供"只勾选与保留项同目录的副本"开关回到保守行为；不参与审阅的备份目录用图片扫描黑名单排除。

> **2026-09-20 修订**（[清理候选检测修复](../2026-09-20-cleanup-detection-fix/概要设计.md#4-决定)）：本节的近似重复实现有两处被推翻。**D-CD03**：单段 16 位前缀分桶改为 8 段 × 8 位（实测库内距离 2~8 的图片对只有 60% 能进入比对）。**D-CD04**：`StaleHashCount` 改为「未回填 + 失效」，原先「无哈希不计 stale」使界面无法提示补全。同时 **D-CD01** 把 dHash 从点采样改为面积平均，**D-CD02** 一次性清空既有指纹重算。汉明阈值 8、邻居/候选上限、连通分量成组、Original 选择规则、默认勾选规则均不变。
...
| 网格虚拟化性能优化时机 | 用户 | 不阻塞 | 首版实测后再定 | closed：2026-08-07 用户裁决纳入，已由 P-013 交付（万级实测窗口内 <100 张卡片） |
| EXIF 拍摄时间是否立项 | 用户 | 不阻塞 | 后续独立 clarify | closed：2026-08-07 用户裁决全量含 GPS，已由 P-012 交付 |
...
| TC-4 | AC-5/AC-6 | D-009/D-010/D-011 | **2026-08-18 重写**：打标状态机单测（mock client，含闭合词表、指纹幂等、取消、外发边界 fail-closed）；候选审阅单测；索引/检索单测（mock embed + pgvector 集成测试跟随现有语义测试形态）；视频页不混排断言 | automation | 无 |
...
| D-009 | AC-5 | data/interface | **2026-08-08 撤销、2026-08-18 重立并实施**：产出标签候选，走与视频一致的候选/审阅流程，三张图片侧表（见 4.6.2）。原契约为描述单表 + 中文 80–200 字禁标签 | 无 run 历史表；不进 AI 质量评估 | 状态机可续跑、脱敏落库 |
| D-010 | AC-6 | data | 共享 SemanticIndexProfile；镜像三件套表 + image_semantic_vectors（HNSW cosine）；索引文本=标题+标签（**2026-08-08 修订、2026-08-18 实施**：原为标题+标签+描述且无描述 Skipped，见 4.6.6） | 图片不触发 rebuild | 全局语义任务互斥 |
| D-011 | AC-6 | behavior/interface | 照片页内 offset/limit 语义检索，limit+1 探测 hasMore | 视频页不混排、全局搜索不含图片 | 混排断言测试 |
| D-012 | AC-9 | behavior | 精确=size+采样MD5 实时；近似=缩略图 dHash 汉明≤8 + 前缀分桶限流；独立 dismissals 表；删除走回收站 | 无 LowDuration/LowResolution/同源 | 精确重复默认勾选多余副本、近似重复不勾（2026-08-08 修订，原为一律不勾） |
...
| D-015 | AC-2 | behavior | 照片页网格（60/页）+ lightbox 查看器 + v-if 挂载；**2026-08-07 修订**：已加二维虚拟化与时间线年月分组（P-013），分页改为按滚动位置预取 | 分组头穿插而非吸顶（吸顶与恒定 DOM 不兼容）；不改 `virtualList.js` | 竞态 token、占位降级、视频列表虚拟化零回归 |
