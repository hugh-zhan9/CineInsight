...
- **手动翻译已有字幕（2026-09-13 裁决）:** 片库行菜单「翻译字幕…」对视频同目录已有的 `.srt` 单独发起翻译，目标语言与输出形态（仅译文 / 双语对照）每次指定；不碰 ASR，也不排字幕生成队列。产物**覆盖同名 `.srt`**——用户裁决不新建 `名称.zh.srt` 这类多语言文件，以免动到字幕索引、重命名跟随、文件迁移与 Jellyfin 暴露这一串既有消费者。翻译先落临时文件，翻译或写回任一步失败都保留原字幕；双语合并后会把文件权限恢复成翻译前的值（`mergeBilingualSRT` 固定写 0644 是给新生成字幕用的，不该放宽用户已有文件的权限）。翻译期间可取消（`CancelSubtitleTranslation` 按 videoID 登记 cancel func），取消后原字幕不动。**空译文保留原文**：LLM 常对拟声词、音乐符号、纯标点行返回空串，写出的空块会被 `parseSRTEntries` 与编辑器解析器整条丢弃——仅译文模式下等于永久删除该条字幕，双语模式下会让其后所有译文错位；该修复在 `translateSRTWithProgress` 内，生成流程的自动双语同样受益。回退发生在喂上文滑窗之前（否则「原文 → 空」会作为示范样本进入下一批提示词），回退后的条目在双语合并时只写一行（`mergeBilingualSRT` 遇到译文与原文相同就不再重复），回退条数会作为 warning 回报给用户——否则「一条都没翻成」和「全部翻成了」是同一个成功提示；这条 warning 两条路径共用 `subtitleFallbackWarning`，生成流程的自动双语同样回报。取消只能在两批之间或请求飞行中生效；`lockSubtitleFile` 是裸互斥锁不认 ctx，所以「排队等锁」期间点取消会记账、但要等拿到锁才返回（拿到锁先复查 ctx，不会再发翻译请求，也不会改文件）。设置页的翻译服务配置（provider / Key / 模型）已移出「双语」开关，因为两个入口共用同一份配置。前端对「多数条目已是多行」的字幕给出叠加警告（覆盖语义下重复翻译会在译文上继续叠加）。
...
- **片库顶部按主题找标签（2026-09-23）:** 顶部筛选区按标签的 `namespace` 切换主题，主题按钮显示标签数和已选数，未设置主题归“未分类”；跨主题多选和清除沿用原筛选语义。视频行标签不分组。标签与分类在同一个「标签管理」页面维护，不再区分普通/AI 标签；分类由实际标签派生，新建时至少指定一个标签，删除分类只让标签归未分类。自动标签的名称与分类不可手动改。
- **自动配色:** 创建标签时自动从 12 色预设调色板中轮换分配颜色，用户无需手动选色。
- **透明度显示:** 标签背景色渲染时自动加 35% 透明度（hex→rgba），保证深色文字清晰可读。
- **搜索过滤:** 添加标签弹窗中输入框同时支持创建新标签和实时过滤已有标签。选中或成功创建并添加标签后清空输入；手机网页端成功关联后同样清空，失败保留输入。
- **软删除恢复:** 创建同名已删除标签时自动恢复（清除 `deleted_at`），避免唯一约束冲突。
- **改名防冲突:** 改名时检查活跃标签和软删除标签，自动清理废弃记录。
- **标签合并:** 管理页统一选择任意非自动保留目标和同义来源，不再有普通/AI 类型筛选；视频、图片关联及既有视频 AI 审批历史/短视频偏好归并，来源随后软删除。视频待审候选转移目标并同步名称，不因历史普通身份失效；图片来源待审按既有策略作废。自动标签不参与。
...
- **短视频和低清自动标签（2026-09-23）:** “短视频”沿用短视频 Feed 最大时长规则；“低清”使用视频已探测高度 `0 < height < 1080`，恰好 1080 和未知高度不自动标注。两者分别以 `automatic_kind=short_video` / `low_resolution` 保持固定身份，启动、扫描、迁移和元数据刷新共用自动对账路径。用户可逐视频手动增删这两种标签，选择写入 `video_automatic_tag_overrides`，后续自动同步遵从该人工决定，直到再次手动改动。若已有同名人工/AI 标签，旧标签保留 ID 和全部关联并安全让名，不被自动规则接管；AI 打标判断忽略自动标签。
- **统一标签库（2026-09-23 用户裁定）:** 用户明确“统一管理，全部标签都可供 AI 选择”。视频/图片 AI 加载、提示词、匹配、审核都使用全部未删除的非自动标签（`automatic_kind` 为空或历史 NULL），不再按 `is_system` / `is_active` 筛选，包括旧停用标签；旧列保留而不迁移 ID 或重建关系。不 seed 词表，空库暂停，出集结果始终丢弃。标签逐项新建/编辑/删除，取消独立 AI 库编辑器和整库保存。标签与分类写入在原 `TagService` 事务中同步任务重排和候选校验；改名同步视频候选名称、图片候选作废，删除使两侧候选失效。旧 AI 库绑定暂保留历史字段读写，但不再有界面调用，不能限制 AI 资格。设置页仅维护模型和任务配置。完整裁定见 `docs/loopx/design/2026-09-23-tag-category-management.md` D-001～D-004。
...
- **手机端标签面板（2026-09-07）:** `POST /short-api/tags {name}`（同源校验 + 严格 JSON 体，与其他写操作同一套防线）→ `ShortFeedService.CreateFeedTag`：去首尾空白、同名直接复用（含软删除恢复，走 `TagService.CreateTag`）、空名 400 `tag_name_required`、超过 64 个字符 400 `tag_name_too_long`、含控制字符 400 `tag_name_invalid`、撞自动标签名 400 `automatic_tag`；数据库错误只回「创建标签失败」，原文进日志。这是第一个由局域网客户端往全局表写任意字符串的入口，长度与字符限制不能省。面板每次打开都重拉标签列表（之前只在首次为空时拉，桌面端新建的标签要刷新页面才出现）；搜索无精确匹配时给「＋ 新建「xx」并添加」，建完直接挂到当前条目。`.sheet-input` 改成在面板底色上可见的框、字号 16px（避免 iOS 聚焦自动放大）。2026-09-08：搜索改为实时查服务端（`GET /short-api/tags?q=`，名字子串不区分大小写、LIKE 转义、超长 400 `tag_query_too_long`；前端 100ms 合并按键、按序号丢弃过期响应）；列表含自动标签并带 `automatic` 标记，面板只读展示、点击无效；`ListFeedTags` 不再按 `automatic_kind = ''` 过滤——该列是后加的可空列，老库标签为 NULL 会被整批漏掉（另一台机器上"标签不全"的真正根因）；面板改 `tall`（94% 高）、已打在当前条目上的标签排前、溢出时底部渐隐提示「还有更多」；舞台标签行同样加溢出提示。
- **手机端 GIF / 横屏素材 / 全屏（2026-09-09）:** ① GIF 不动的根因在 `ImageThumbnailService.ResolveImageFeedView`：超过 3 MiB 的图一律降采样成 JPEG 视图缓存，动图只剩第一帧；现在 MIME 为 `image/gif`（含老库 Format 为空按扩展名判定）时不论大小直出原文件。桌面端图片查看走的是同一套 view 缓存，大 GIF 在桌面同样是静态的，本次未改。② 横屏素材：`.feed-video` 一直是 `object-fit: cover`，16:9 视频在竖屏手机上只剩中间三分之一。`FeedStage` 现在给横屏视频加 `feed-video--landscape`（contain）：先按后端 `width/height` 预判，`loadedmetadata` 后以 `videoWidth/videoHeight` 为准（它算进了旋转元数据，手机拍的竖屏视频在快照里常是 1920×1080），换条目重置、同条目被后端回传覆盖不重置；`@media (orientation: landscape) and (pointer: coarse)` 下竖屏素材同样 contain（只限触屏：桌面窗口几乎总是横向，≥760px 的 cover 规则保持不变），放在文件末尾压过前面的规则。③ 全屏：顶栏新增按钮（`data-test="short-feed-fullscreen"`），能用元素全屏（安卓 Chrome、iPad、桌面）就对 `document.documentElement` 进全屏，面板、动作栏与滑动切换都留在里面；iPhone Safari 没有元素全屏，退到 `<video>.webkitEnterFullscreen()`（系统播放器，退出后回到本页），图片条目上按钮不出现；状态跟随 `fullscreenchange`。`short.html` 加了 `apple-mobile-web-app-capable` 等 meta，从主屏幕图标打开即无地址栏，这是 iPhone 上唯一的整页全屏方式。进全屏以 `document.fullscreenEnabled` 为准而不是方法是否存在（iPhone 上方法可能存在但永远被拒），iPhone 系统播放器的进出由 `<video>` 的 `webkitbegin/endfullscreen` 事件回写状态。回归：`frontend/src/components/FeedStage.test.js`（横竖屏判定）、`ShortFeedFullscreen.test.js`（整页全屏/无路径不显示/iPhone 回退）、`short-feed.test.mjs` 追加的源码断言、`TestShortFeedAnimatedGIFIsAlwaysServedAsOriginal`。
...
- **分页后的局部更新规则:** 批准、拒绝、整媒体拒绝、重新分析都按后端实际改动的行局部移除（同媒体同标签 ID 候选一并 `superseded`；该媒体已有手工标签时整媒体作废；`RetryVideo` 把该视频待审候选全部 `superseded`），因为整表重拉会把已翻出来的页丢掉、把用户拉回列表顶部。仍需真正重取的动作（手动刷新、手动加标签、同源删除、候选过期报错）显式回到第一页。当页被移空但后端还有下一页时自动补一页，避免"没有待审候选"的假空态。
...
- **「算看完」的统一口径（2026-09-13 裁决）:** 用户看到列表里「看到 00:28 / 00:28」却没标已看，裁定这就是已播完，且已播完的下次要从头播。判定收在 `VideoService.UpdateVideoWatchProgress` 一处：位置离片尾不到 `watchedCompletionToleranceSeconds`（1 秒）即视为看完。裁决要求**严格**，所以用绝对容差而非 5 秒 / 95% 这类宽松阈值；容差再取 `min(1, duration*0.05)` 并要求位置 >0：固定 1 秒在亚秒片段上会让「位置 0」满足 `position >= duration-1`，而取时长一半又等于「看过一半就算看完」——比被否决的 95% 还松。20 秒是比例与绝对值的交接点。判定成立就标已看**并把断点清零**——这一条同时兑现三件事：下次从头播、列表不再显示「看到 X / Y」、与 Jellyfin 自身语义（`Played=true` 配 `PlaybackPositionTicks=0`）一致。`watched_at` 只记第一次看完的时间。三条上报路径都收敛到这个口径：内嵌播放器（ended 仍可显式传 `completed`，时长元数据不准时它可能远离片尾）、Jellyfin `/Sessions/Playing/*`（此前自己要求 `seconds >= duration`，比应用内严，同一部片在两个客户端结论不同）、**IINA 断点同步**（桌面「播放」按钮走的就是它，此前完全不动 `is_watched`，不改这条的话用户报的现象在主力路径上根本没修掉）。IINA 同步另加三条：已看的视频直接跳过（位置清零后「只前进不后退」挡不住陈旧 `watch_later` 记录把它拉回「在看」）；断点文件**消失**仍不据此判已看（可能是用户自己清了记录）；**完成判定必须排在「只前进不后退」守卫之前**——两者幅度同为 1 秒会形成死区，既吞掉正常完成（库里存 26.6、这次到 27.3 就被跳过），也让历史遗留行恒满足 `seconds <= position+1` 而永不自愈。`IINAProgressUpdate` 带 `Watched` 字段，前端就地补「已看」徽标，不必等整页重载。手动 `SetVideoWatched(true)` **有意不动断点**：误点一下就把两小时电影的断点抹了且撤不回来。已看视频被上报中途位置时也照常记录（重看场景）。对外自洽改在一处解决——`jellyfinUserData` 在 `Played` 时把 `PlaybackPositionTicks` 报成 0，与 `IsResumable` 的筛选口径（`is_watched=false AND position>0`）对齐；桌面端本来就不显示也不续播。前端 `resumePositionFor` 的容差同步收到 1 秒（原为 `max(duration-5, duration*0.98)`，长片上反而更宽松），行组件 `watchProgressPercent` 在 `is_watched` 时返回 0。
- **历史遗留记录不回填（2026-09-13 裁决）:** 库里存在 `watch_position_seconds >= duration-1 且 is_watched=false` 的老记录（旧 IINA 夹紧、或旧代码「看完后手动标未看」留下的）。用户裁定**不做一次性回填**，让它们在下一次进度上报时自愈。理由是这两类记录在数据上无法区分，回填会把用户显式标过未看的片子又标回已看。代价是更新后这些行仍会显示「看到 X / Y」，直到再播一次。
...
- **扫描目录黑名单:** 设置页按目录维护扫描排除列表；黑名单目录及其全部子目录会在启动扫描、手动目录扫描、增量扫描和迁移候选扫描中跳过。加入黑名单停止后续收录，并在查询时隐藏已有视频；不删除记录，取消排除后恢复可见。
- **扫描恢复与候选过滤（2026-09-10）:** 全量扫描找到 `is_stale` 旧记录时也清标记并计入 `Restored`，与窄对账一致。两条路径的未命中候选必须经 `os.Stat` 明确不存在才能进入迁移/删除/标失效，近期活跃、隐藏或排除规则不等于磁盘丢失。目录遍历错误向上传，阻止失败根参与缺失对账；已明确排除、隐藏或回收站目录仍优先跳过（`filepath.Walk` 在回调前尝试读取目录，不能让不可读的 `.Spotlight-V100` 挡住全盘扫描）。跨目录旧失效记录只在真正匹配新文件后检查存在性，避免无关挂载盘错误污染本地实时同步。手动增量扫描摘要增加「恢复显示」，复用 `TaskFailureList` 展示失败路径和原因。用户手动删除与来源未知的历史删除不自动复活；明确由扫描删除的记录按下述 2026-09-11 规则恢复。另一台 Mac「手动扫描 0、无报错」及本地 / SMB 文件不可见仍待现场验证，不能由上述回归推定已解决；跟踪见 `docs/issues/2026-09-10-scan-zero-on-second-mac.md`。
...
- **对账收拾孤儿记录（D-S01/D-S04 的历史遗留补充）:** 只在删目录那一刻处理是不够的——旧版本删目录只删配置行，那批记录已经掉进夹缝。`SyncScanDirectories` / `SyncImageDirectories` 末尾各跑一次 `reconcileOrphanedVideos` / `reconcileOrphanedImages`：不属于任何**已配置**目录的记录，视频与图片都只标失效、由查询过滤，不软删。判据用配置目录而非本轮扫到的 `roots`——盘没插时根扫不了但仍配置着，底下的记录只按离线标失效，不软删；目录清单读失败时直接返回错误、绝不走到这一步，否则会把整库藏起来。
...
- **扫描删除边界与删除人（2026-09-11，用户最终裁决）：** 磁盘退出或加入黑名单只隐藏；只有成功扫描的根仍在线、路径未被排除且 `os.Stat` 确认文件不存在时，全量扫描才软删除。`scanRemovalGuard` 保留遍历前根目录身份，缺失候选进入迁移前、删除前和扫描结束都核对在线状态；macOS 对 `/Volumes/<盘名>` 额外核对实际挂载点，防止退出后残留空目录伪装成在线。权限/遍历错误不得当作文件丢失。图片移除扫描范围也改为只标失效，默认图库/语义搜索/清理/手机候选过滤失效项；图片黑名单沿用独立配置为空时回退通用黑名单的语义。
- **统一手动扫描对账：** 手动目录弹窗原先在前端按扫描列表求差，再调用 `DeleteVideo`，会把程序操作混入用户删除，且漏掉后端文件存在性保护。现由 `SyncDirectoryWithProgress` 共用 `syncScanDirectories`，只处理选中目录，不对其他根运行孤儿对账；保留发现/处理事件和请求隔离，并展示恢复、删除、暂不可用与失败结果。
- **删除归因与自动恢复：** `videos`、`images` 和两类回收站条目新增 `deleted_by`，`user`=用户操作、`scanner`=程序扫描，旧行默认空值表示「历史未知」，不猜测回填。归因与软删除同事务落库，视频中断删除续跑从回收站条目继承；回收站显示删除人。启动/全量/窄范围/手动目录扫描找到 `scanner` 删除的视频时，用原回收站条目恢复原 ID、标签与字幕索引；只有确认是扫描删除且删除时文件已缺失、没有内容指纹的条目，才允许按原路径与记录大小恢复。用户/历史未知删除不自动恢复；图片旧扫描已有 `is_stale` 来源标记继续兼容，但明确 `user` 永不自动恢复。回收站恢复同时清 `is_stale`，避免恢复成功仍不可见。扫描新增来源记录后沿用备份与迁移。
...
- **清理审阅决定与缓存（2026-09-20）：** 「不是同片」「不是同源」「截取片段忽略」原本已有持久记录，但重开面板直接回传分析缓存会让已处理项复现。`cleanup_review.go` 统一在 `CleanupService.Status` 回读与完整分析结束前按数据库决定过滤，读取失败明确报错、不回传未经核对的候选；过滤创建新切片，不修改共享缓存。近似重复组内有已否决配对时拆成互不重叠、两两相容的组。「不是同片/同源」同时排除截取候选，避免换类别复现；只忽略截取仍不等于否认同源。截取缓存保留计算时的私有源指纹，与 `ClipDismissal` 按原有文件版本规则匹配，重编码后的新分析仍可重新评估。前端保存成功提示「已记录」并回读过滤后的状态，按 ID 移除已处理项及失效选择，请求序号阻止保存前发出的迟到响应恢复旧行。既有决定直接复用，无需用户重做。
...
- **人物详情图片比例（2026-09-14）：** 人物主页与预览抽屉的关联图片均按缩略图自然宽高比展示（宽度随卡片、高度自动），不再强制正方形裁剪；卡片顶端对齐，内部网格使用 `minmax(0, 1fr)`，避免长文件名撑宽图片与操作区。
- **人物作品数量与媒体删除（2026-09-14）：** 人物卡片分别显示 `active_video_count` 部视频与 `active_image_count` 张图片，详情顶部也分别报告加载量；原卡片只读视频数却称「活跃作品」，图片型人物因而显示 0。人物详情页、编辑抽屉和图片放大预览可删除对应媒体，复用 `DeleteVideo` / `DeleteImage`；共用 `PersonMediaDeleteDialog` 确认文件名、路径、大小，默认保留磁盘文件，可勾选原文件移入回收站。成功就地移除媒体并更新数量、清除选择与失效分页响应，失败保留原项供重试；删除媒体不解除人物关系、不连带删除人物。抽屉向宿主页发 `media-deleted` 同步列表，主片库收到视频删除时也刷新。
...
- **近重复指纹状态文案（2026-09-20）：** 状态条显示「本轮新生成 / 已有指纹复用 / 处理失败」，排队期间失效、删除或移出范围另显示「已移出处理范围」。`PerceptualHashStatus.reused` 与 `out_of_scope` 分别计数，旧 `skipped` 保留为两者合计。指纹补全数量不是近重复候选数量，状态条指引去清理中心查看候选；有失败时显示「本轮结束，有失败项」，并说明部分失败视频仍可播放。
...
- **匹配（2026-09-22 误判优化）:** `services/clip_match.go` 保留 0.9 时长比与 10 秒下限，汉明容差改为 8、有效命中率 80%；均匀抽最多 16 帧粗筛。低信息指纹不计命中，证据需分布在四个时间区间且包含至少三种不同画面。`services/clip_verify.go` 对可能成为最佳候选的配对进一步抽取最多 5 对 32×32 本地画面，以归一化亮度结构做全图和分块复核；只有结构复核通过才展示。用户明确调色后的片段也要识别，因此颜色、亮度差异不直接否决；回归覆盖亮度/对比度/色调/饱和度、黑白与暖色调。抽帧只驻内存，不调用外部 AI、不新增存储，已有哈希无需重算；每对读取有 30 秒期限；2026-09-24 按用户反馈改为超时、读失败或源指纹变化只跳过当前配对，`skipped_clip_verification` 在清理面板提示本轮跳过的配对数，日志记录双方 ID 与脱敏原因，重新分析可重试；不返回未经复核的候选。应用上下文取消仍终止整轮，数据库错误仍报分析失败。静态证据不足不出候选；仍不承诺倍速、镜像、空间裁剪。完整规则见 [D-027](docs/loopx/design/2026-09-02-capability-batch/需求设计文档.md#D-027)。
...
### 2.26 人脸审阅与人物候选
- **逐条移除来源（2026-09-14）：** 「待命名人脸 · 来源详情」每行可移除一次人脸观测（`RemoveFaceClusterObservation(clusterID, observationID)`），仅允许未命名簇。移除将该观测的 `cluster_id` 置空、追加状态置 `dismissed`，保留源指纹和原媒体，源未变化时不会重新分析；不删除文件、片库记录或人物关系。同事务重算次数、代表头像和剩余向量的中心，空簇连同候选清除。移除与新观测聚类串行，聚类前重读簇快照，避免后台分析使用刚被移除的簇。前端就地删行、失败保留，当前页移空且有下一页时自动补页，空簇关闭详情并刷新列表。
- **服务:** `services/face_review_service.go`（`FaceReviewService`，命名时依赖受控头像存储）。`ListFaceClusters(filter{status, media_kind})` 返回簇卡片：代表观测 id（前端拼 `/preview/face-crop/{id}`）、观测数与涉及视频/图片数（按观测行现算）、状态、已关联人物、候选人物、pending 追加数与涉及媒体名；按观测数降序。
...
- **前端:** `FaceClusterReviewPanel.vue` 一个组件两处复用——视频侧 `AITagReviewDialog.vue` 第三个页签「人物候选」（`face-cluster-review-tab`），图片侧 `ImageAITagReviewPanel.vue` 顶部页签（`image-ai-tag-review-tab` / `image-face-cluster-review-tab`）。未命名簇三动作（命名新人物 / 关联现有人物（候选人物置顶）/ 忽略）；已命名簇只在有 pending 追加时出现（确认追加 / 忽略追加）。订阅 `face-review-changed` 与 `face-analysis-state` 局部刷新，刷新失败保留已有卡片。**卡片分批渲染（2026-09-07 修卡死）：** 簇数没有上限（每张没匹配上的脸都是一簇），面板只渲染前 20 张卡片（`CARD_PAGE_SIZE`），「显示更多（还有 N 组）」每次再放 20 张；窗口在事件刷新与动作后重拉时不缩回；回报宿主的 `loaded` 计数始终按全部簇算，与窗口无关。已知限制：后端 `ListFaceClusters` 仍一次返回全部簇（无服务端分页）、每次动作后整体重拉。**第三个入口（用户裁决 2026-09-07）**：人物页（`EntityLibraryPage`，`entityType=person`）列表视图顶部常驻「待命名人脸」分区，复用同一面板；面板加载后 `emit('loaded', { unnamed, appendPending })`，宿主据此写摘要、在没有待处理项时默认收起（用户手动切换过就不再自动收），面板 `changed` 触发人物列表重拉——命名/关联出来的人物立刻出现在下面的列表。
- **来源详情与头像（2026-09-11）:** 人脸卡片「查看来源详情」→ `FaceClusterDetailDialog`；`GetFaceClusterObservations` 按观测 ID 游标分页（默认 50、上限 200；UI 每页 30），分媒体类型批量读原记录、返回文件名/路径/大小/帧时间，不带向量/裁剪磁盘路径、不启动扫描。删除、stale、已移除来源显示原因；原图复用 `ImageSourceDialog`，视频复用 `PreviewDrawer` 并传观测毫秒数（0 是有效首帧）。预览返回保留分页，Escape 只关最上层。命名时在同一数据库事务里复制代表裁剪图到人物托管头像目录，清除人脸缓存不影响头像；裁剪图不存在仍可命名，保存失败回滚，失败事务清理无引用的新文件。既有缺头像人物可运行 `go run ./cmd/repair_face_avatars --env <配置路径> --data-dir <应用数据目录> --apply` 修复（省略 apply 只统计；写入前保存 ID/旧头像路径快照，只填空头像）。刻意不在每次启动回填，尊重用户之后主动移除头像。
- **图片操作（2026-09-11）:** 图片库文件夹与单图详情支持打开目录；人物页及详情抽屉的关联图片可点击放大，放大页支持目录与解绑，人物页另有直接解绑按钮。复用 `RemovePersonImage`，跨媒体最后一条关系的孤立人物清理仍先确认。人物页/抽屉多选图片后由 `ImageBatchTagControls` 批量添加既有非自动标签，失败保留失败项便于重试；图片库保留既有多选标签逻辑并增加明确按钮。图片库预览删除当前图后优先下一张（必要时读下一页），末张回到前一张，仅删空时关闭；失败保留当前预览并在预览内显示错误。
...
- **详情、搜索与删除（2026-09-09，设计 V1.0.3）：** 用户真机反馈 Fileball 详情页「视频:」「音频:」为空、搜索不可用、没有删除。根因：① 流 DTO 的 `DisplayTitle` 原来照抄 ffprobe 的 Title（几乎总是空），而 Jellyfin 是服务端合成的（视频「1080p HEVC SDR」、音频「Dolby Digital+ - 5.1 - Default」），且 Fileball 从条目顶层 `MediaStreams` 读；现在 `jellyfin_stream_dto.go` 按 `MediaStream.GetDisplayTitle` 规则合成（分辨率档位按长短边，竖屏 1080×1920 仍标 1080p，不像 Jellyfin 标成 4K），附图流为 `EmbeddedImage`，条目顶层重复 MediaStreams 并给 Container/Width/Height/AspectRatio/IsHD/HasSubtitles/VideoType/PlayAccess/CanDelete/CanDownload；没有快照的视频这些字段为空，不臆造。② 搜索：新增 `Search/Hints`（复用 `listItems`，SearchTerm 必填、强制 Recursive、IncludeMedia=false 判空），`IsMovie/IsSeries/IsNews/IsKids/IsSports`（本库叶子全是 Movie，所以是全集或空集）与 `NameStartsWith/NameStartsWithOrGreater/NameLessThan`（比较小写 SortName）归入语义参数，`Include{People,Media,Genres,Studios,Artists}` 归入投影提示；详情附属接口 `Items/{id}/{SpecialFeatures,LocalTrailers,AdditionalParts,ThemeSongs,ThemeVideos}` 空数组、`{Items,Movies,Shows}/{id}/{Similar,Intros}` 空页（原来 404，Fileball 日志里确有 SpecialFeatures 404）。注意这些附属路由只在 `parts[2]` 命中已知名字时才解析 ID，否则仍 404——伪造路径的日志脱敏测试依赖这一点。③ 删除是**用户裁决**改掉 D-01 的「不能删除媒体」：`DELETE Items/{id}` 只接受视频，二次鉴权同收藏/进度但**不持 `s.writes` 锁**（移回收站要先整文件 SHA-256，几 GB 的片会把别的客户端的进度回报卡住；VideoService 自己用 `libraryPathMutationMu` 串行化），调 `VideoService.DeleteVideo(id, true)` 进回收站（桌面可恢复），文件夹 403、无效 ID 400、不可见 404；详情附属接口同样先校验条目可见（不存在 404），但不校验查询参数——答案恒空，没有"错误范围"可守；Policy.EnableContentDeletion 与条目 CanDelete 置 true（Fileball 靠这两个显示删除按钮）。失败只记视频 ID 不记错误原文（原文含路径）。遗留：Jellyfin 本身允许匿名取 `Items/{id}/Images/Primary`，日志里有 1 条无令牌封面请求被 401，D-01 仍要求令牌，待裁决。
...
### 2.29 想看片单（2026-09-10）
- 顶栏「想看」与命令面板「打开想看」维护未入库的片名备忘。只填片名，支持添加、搜索、改名和手动移除；用户明确选择手动移除，扫描入库与下载不自动修改片单。
- `watchlist_entries`（`models/watchlist.go`）只存 ID、片名及创建/更新时间，无媒体外键；去首尾空白后片名唯一（大小写仍区分）；新增/改名冲突提示「该片名已在想看片单中」。`idx_watchlist_title` 在数据库防止并发重复写入；升级先在事务内归并历史同名项，保留最早 ID 和创建时间，再建索引。`WatchlistService` → 当前 GORM 数据库，加入 `AllModels`，沿用备份和后端迁移。片名去首尾空白后 1–200 字符，搜索按片名匹配、转义 LIKE 特殊字符；ID 倒序游标分页。移除只删指定片单记录，不操作视频或磁盘文件。
- 桌面入口 `app_watchlist.go` / `WatchlistPage.vue`；页面提交期间禁用重复动作，失败保留输入和已加载内容，过时搜索响应丢弃。设计与验证见 `docs/loopx/design/2026-09-10-watchlist/需求设计文档.md`。

- **在线补全（2026-09-11，设计 `docs/loopx/design/2026-09-11-watchlist-online-metadata/`）：** 条目新增 `kind` 维度（`movie`/`tv`/`anime`/`show`/`av`），唯一约束由 title 改为 **(title, kind)**——同名不同类型可共存，同名同类型仍报「该片名已在想看片单中」。**索引的列顺序与名字前缀是承重的**：`watchlistTitleConflict` 靠字符串匹配识别撞名，调换列顺序或改成不以 `idx_watchlist_title` 开头的名字会让撞名静默退化成通用数据库错误。旧的 `migrateWatchlistTitleUniqueness` 已退役为 `dedupeWatchlistTitlesBeforeSchema`（仍在 `AutoMigrate` 前跑、仍按 title 去重、**不再建任何索引**，守卫首要条件是"库里还没有 kind 列"）；新迁移 `migrateWatchlistKind` 跑在 `AutoMigrate` **之后**，无表锁。两者方向相反，别搞混。
- 补全按类型路由：movie→豆瓣→TMDB，tv/show→TMDB，anime→Bangumi 走**链**；av→**JavBus + jav321 + FC2 走聚合**。两种策略并存，分流点唯一，在 `lookupWatchlistDetail`（`services/watchlist_enrichment.go`）。走链＝第一个成功就停，**兜底只在源明确 `not_found` 时触发**，凭证/代理/网络错误一律不兜底——否则配置问题会被伪装成「查无此片」；走链只有一份：`walkWatchlistMetadataChain`（`watchlist_metadata_chain.go`）。聚合＝**并发问所有源、逐字段择优**，表在 `watchlist_metadata_aggregate.go`，全失败时按「最值得用户处理」排序上报（全 `not_found` 才报 `not_found`）。适配器不碰数据库、不反向依赖 `WatchlistService`。
- 补全在后台异步跑（任务 key `watchlist_enrich`，**不过空闲门**，因为是用户添加或重试触发的）。认领用条件更新 `WHERE id=? AND enrichment_status='pending'` 加 32 字符随机标识，写回再带 `status='running' AND claim=<本次>` 守卫；**禁止 `SELECT ... FOR UPDATE`**。守卫不成立＝用户期间编辑或删除了条目，**丢弃结果并记日志，不报错**。`EnrichmentClaim` 是 `size:32`，`uuid.NewString()`（36 字符）会在 SQLite 静默截断、PG 报错。失败分六类（`credential_missing`/`credential_invalid`/`proxy_unreachable`/`network_unreachable`/`not_found`/`source_error`）**不许合并**，`credential_missing` 不发任何请求。进度经 Wails 事件 `watchlist-enrich-progress` 推送，前端据此自动刷新该行。
- 海报复用 `ManagedImageService`（entityType 白名单加了 `watchlist`），落 `dataDir/media-details/watchlist/<id>/<sha256>.<ext>`，经 `/preview/watchlist-poster/<id>` 取；**先落盘再写库**，写回失败即删图。
- 凭证与代理在设置页「在线资料源」分区，代理项叫 **「资料源出网代理」**——与既有 `ProxySection.vue` 的「播放代理」（本地转码缓存）无关，别混。`Settings` 有 3 列在线资料源配置（FANZA 的 2 列已随该源退场，**列仍留在库里**——迁移器不删列，删列会破坏回退），**必须同时进 `settings_service.go` 的 `UpdateSettings` 白名单与 `SettingsPage.vue` 的 `UpdateSettings({...})` 显式载荷字面量**，漏任一处都会让保存静默失效或把已存值清空。配置优先级：**Settings 的非空值覆盖环境变量**（`services/ai_tagging_config.go:80` 的既有方向）。
- **验证状态**：JavBus 与 jav321 已用真实番号（SSIS-001）端到端验证，字段逐项核对通过；TMDB / Bangumi 的字段映射仍只来自公开文档、无真实请求。PostgreSQL 侧迁移与撞名文案**已实测通过**（postgres:18-alpine）；注意 `./services` 在 PG 后端要约 650s，超过 `go test` 默认 600s 超时，跑 PG 全量必须加 `-timeout`。**JavBus 必须带 `dv=1` cookie**，否则源站 302 到 `/doc/driver-verify`，Go 跟过去拿回解析不了的 200，每次查询都落 `source_error`（2026-09-13 查实并修复）。**FC2-PPV 由 FC2 官方站（adult.contents.fc2.com）单独负责**：它是用户投稿平台，作品不在片商目录里，JavBus 与 jav321 都查不到。番号形态的分流**不做成第二张路由表**——三个源都挂在 av 上各认各的番号，FC2 适配器见到片商番号当场 not_found 且不发请求，另两家遇到 FC2 番号自然 404，靠聚合器既有的「单源失败不阻断」收场。**jav321 的 `/video/` 只认 DMM 补零编码**（`ssis00001`），标准番号要先过它的 `POST /search` 由源站自己换算——我们这边不做换算。两个源都是正则解析 HTML，源站改版即失效（落 `source_error` 不落 `not_found`）。**海报下载必须带 `Referer`（图片自身站点根，末尾斜杠不能少）**：JavBus 的图床在 Cloudflare 后面做防盗链，不带就 403，详情页能取但图取不到（2026-09-13 查实并修复）。**airav 挡在 Cloudflare JS 挑战后面，接不了**；freejavbt 实测 8 个番号只有 3 个有中文标题且其中 1 个是错片，数据质量不达标，未采用。
...
- 海报**落盘**（2026-09-22 起）：由详情补全 worker 随详情一并下载，经 `ManagedImageService`（entityType `movie_chart`）落到 `media-details/movie_chart/<id>/<sha256>.<ext>`，条目上的 `poster_path` 记托管相对路径。只读路由 `/preview/douban-chart-poster/<豆瓣ID>` 是**纯本地磁盘读、零出网**，未命中返回 404 由前端显示占位，**不即时回源**。白名单（https + `doubanio.com`，写成两段判断以拒 `evildoubanio.com`）、`CheckRedirect` 逐跳复检、Referer 取图片站点根（**末尾斜杠必需**）与浏览器 UA 全部搬到下载侧 `services/movie_chart_artwork.go`。
- **为什么改**：初版按「不落盘、即时转发」交付，靠响应头 `Cache-Control: max-age=604800` 兜重复访问。真机一天内豆瓣图床开始返 403/418。根因两层：渲染路径每次都回源；而那个缓解手段**从未生效过**——海报经 Wails `AssetServer.Handler`（`main.go:41`）即自定义协议提供，**WKWebView 不对自定义协议响应做 HTTP 缓存**。任何「靠响应头给 webview 资源做缓存」的想法在这个应用里都不成立。
...
- **开发模式:** `GOTOOLCHAIN=go1.24.9 wails dev`（Wails 2.11.0 的 `go/packages` 与本机 Go 1.27.1 不兼容；开发模式尚未实测）
- **构建应用:** `GOTOOLCHAIN=go1.24.9 wails build`（2026-09-23 darwin/arm64 已完整出包；默认 Go 1.27.1 在 Wails 扫描嵌入资源阶段报 `package "fmt" without types`，`go test ./...` 不受影响）
- **构建并安装脚本:** `scripts/build_and_install_app.sh` 在 `GOTOOLCHAIN` 未设置或为 `auto` 时只为 Wails 构建使用 Go 1.24.9；`--build-only` 可安全验证构建和 sidecar 打包而不替换 `/Applications` 中的应用。
...
### 2026-09-16 详情与头像补充
- Jellyfin 详情投影现有标签、人物、作品集和个人评分。TagItems 使用数字标签 ID，GenreItems 保持类型化 GUID；人物 ID 前缀 07。人物浏览和封面均复用可见视频范围及托管图片服务。Similar 汇总共享人物/作品集/非自动标签/已检测同源关系，不调用 AI。
- 本地 Filebar 已验证标签、人物、所属作品集与评分显示；当前客户端未提供已验证的数值打分入口、未请求 Similar。客户端搜索建议退出片库后重进可刷新。
- 桌面人物详情新增“从图片库选择头像”和关联图片“设为头像”，保留从文件选择。复用 SearchImagePage 的搜索/分页和 SetPersonAvatar 的托管复制；不修改原图，不自动新增人物关联。
