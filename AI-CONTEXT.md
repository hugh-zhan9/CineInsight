...
- 补全按类型路由：movie/tv/show→TMDB，anime→Bangumi 走**链**；av→**JavBus + jav321 走聚合**。两种策略并存，分流点唯一，在 `lookupWatchlistDetail`（`services/watchlist_enrichment.go`）。走链＝第一个成功就停，**兜底只在源明确 `not_found` 时触发**，凭证/代理/网络错误一律不兜底——否则配置问题会被伪装成「查无此片」；走链只有一份：`walkWatchlistMetadataChain`（`watchlist_metadata_chain.go`）。聚合＝**并发问所有源、逐字段择优**，表在 `watchlist_metadata_aggregate.go`，全失败时按「最值得用户处理」排序上报（全 `not_found` 才报 `not_found`）。适配器不碰数据库、不反向依赖 `WatchlistService`。
