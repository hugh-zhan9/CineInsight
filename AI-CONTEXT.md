...
- **同源审阅与否认:** `video_same_source_relations` 保存规范化视频对、双方内容指纹、置信度、理由、未读状态和处理时间；首页 AI 标签管理按钮显示未读数，审阅弹窗展示双方，单个“预览”操作会同时打开两个视频，并提供“确认同源 / 不是同源”两种处理。确认后关系仍保留给清理中心使用，但退出待处理列表；涉及软删除视频的关系不会展示。用户否认后，只要双方内容指纹未变就禁止再次确认；任一文件内容变化后允许重判。
