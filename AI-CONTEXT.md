...
- 补全按类型路由：movie/tv/show→TMDB，anime→Bangumi，av→**FANZA 优先、JavBus 兜底**。**兜底只在源明确 `not_found` 时触发**，凭证/代理/网络错误一律不兜底——否则配置问题会被伪装成「查无此片」。策略只有一份：`walkWatchlistMetadataChain`（`services/watchlist_metadata_chain.go`），补全侧委托它，别再写第二份。适配器不碰数据库、不反向依赖 `WatchlistService`。
...
- 凭证与代理在设置页「在线资料源」分区，代理项叫 **「资料源出网代理」**——与既有 `ProxySection.vue` 的「播放代理」（本地转码缓存）无关，别混。`Settings` 新增 5 列，**必须同时进 `settings_service.go` 的 `UpdateSettings` 白名单与 `SettingsPage.vue` 的 `UpdateSettings({...})` 显式载荷字面量**，漏任一处都会让保存静默失效或把已存值清空。配置优先级：**Settings 的非空值覆盖环境变量**（`services/ai_tagging_config.go:80` 的既有方向）。
- **未验证**：四家源的字段映射全部来自公开文档，无凭证、无真实请求；PostgreSQL 侧迁移与撞名文案**已实测通过**（postgres:18-alpine）；注意 `./services` 在 PG 后端要约 650s，超过 `go test` 默认 600s 超时，跑 PG 全量必须加 `-timeout`。FANZA 番号→content_id 转换规则无公开文档，查询直接用番号，预期会落到 JavBus。JavBus 为正则解析 HTML，源站改版即失效（落 `source_error` 不落 `not_found`）。
