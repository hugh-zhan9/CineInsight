...
- **片库顶部按主题找标签（2026-09-23）:** 顶部筛选区按标签的 `namespace` 切换主题，主题按钮显示标签数和已选数，未设置主题的标签归“未分类”；跨主题多选及“全部”清除仍沿用原筛选语义。标签管理集中维护普通和 AI 标签库：普通标签用分类下拉，分类管理可新建（须同时指定至少一个标签）、改名、删除，普通与 AI 标签共用；删除分类后标签归“未分类”，不删除标签。AI 标签库从设置页移出，设置页只保留 AI 模型和任务配置。自动标签的名称与分类不可手动改。视频行的标签展示不分组。
...
- **用户配置闭集标签库:** 不内置或 seed 任何 **AI 词表**标签和分类；标签管理中的 AI 标签库按分类维护标签、颜色和启用状态，可含未分类 AI 标签。已有普通标签可原地加入 AI 标签库并保留全部视频关联；用现有名称替换一个 AI 标签库条目时复用该普通标签，旧条目退出标签库但不删除，原主题分类与视频关联仍保留。AI 只从 `is_system=true AND is_active=true` 标签中选择，标签库为空时暂停分析，出集结果服务端丢弃。标签库或 AI 配置保存后会立即唤醒后台任务；标签库变更时仅让已移除、停用或无法匹配的待审候选失效，仍匹配有效标签的候选会保留，标签改名时同步候选名称；没有有效待审候选且无正式标签的视频会按新库重跑。
- **AI 标签库防误清空:** 标签管理只有在 AI 标签库成功加载后才允许保存该库，加载失败时保留错误并提供重新加载；设置页保存不再写 AI 标签库，也不受其加载失败阻塞。普通 `SaveAITagLibrary([])` 不能清空一个已有标签库，必须由前端二次确认后调用独立 `ClearAITagLibrary`。该双层保护避免数据库短暂断连把前端空状态误写为用户主动清空，并连带使待审候选失效。
