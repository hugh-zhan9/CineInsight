...
- **验证状态**：JavBus 与 jav321 已用真实番号（SSIS-001）端到端验证，字段逐项核对通过；TMDB / Bangumi 的字段映射仍只来自公开文档、无真实请求。PostgreSQL 侧迁移与撞名文案**已实测通过**（postgres:18-alpine）；注意 `./services` 在 PG 后端要约 650s，超过 `go test` 默认 600s 超时，跑 PG 全量必须加 `-timeout`。**JavBus 必须带 `dv=1` cookie**，否则源站 302 到 `/doc/driver-verify`，Go 跟过去拿回解析不了的 200，每次查询都落 `source_error`（2026-09-13 查实并修复）。**jav321 的 `/video/` 只认 DMM 补零编码**（`ssis00001`），标准番号要先过它的 `POST /search` 由源站自己换算——我们这边不做换算。两个源都是正则解析 HTML，源站改版即失效（落 `source_error` 不落 `not_found`）。**airav 挡在 Cloudflare JS 挑战后面，接不了**；freejavbt 实测 8 个番号只有 3 个有中文标题且其中 1 个是错片，数据质量不达标，未采用。
