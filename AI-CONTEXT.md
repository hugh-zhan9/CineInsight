...
- `watchlist_entries`（`models/watchlist.go`）只存 ID、片名及创建/更新时间，无媒体外键；同名允许分别记录。`WatchlistService` → 当前 GORM 数据库，加入 `AllModels`，沿用备份和后端迁移。片名去首尾空白后 1–200 字符，搜索按片名匹配、转义 LIKE 特殊字符；ID 倒序游标分页。移除只删指定片单记录，不操作视频或磁盘文件。
