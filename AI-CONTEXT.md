...
- **标签合并:** 标签管理页先明确选择普通标签或 AI 标签类型，再选择保留目标并按名称筛选、复选多个同义来源标签；来源关键词不会反向隐藏目标。视频关联、AI 审批历史引用和短视频偏好权重会一并归并，来源标签随后软删除。普通标签与 AI 标签库标签只允许在各自类型内合并，自动标签不参与合并。
...
- **同源审阅与否认:** `video_same_source_relations` 保存规范化视频对、双方内容指纹、置信度、理由、未读状态和处理时间；待审工作台用独立子页签区分 AI 标签与视频同源，内容区独立滚动。同源页展示 A/B 原始路径，单个“预览”操作会同时打开两个视频，并提供删除 A/B 片库记录（保留原文件）、“确认同源 / 不是同源”处理。确认后关系仍保留给清理中心使用，但退出待处理列表；涉及软删除视频的关系不会展示。用户否认后，只要双方内容指纹未变就禁止再次确认；任一文件内容变化后允许重判。
