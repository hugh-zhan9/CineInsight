...
const api = vi.hoisted(() => ({
  SelectDirectory: vi.fn(), ScanDirectoryWithProgress: vi.fn(), AddVideo: vi.fn(),
...
async function start() {
  ...
  await flushPromises();
  return { running, requestID: api.ScanDirectoryWithProgress.mock.calls[0][1] };
}
...
describe('手动目录扫描实时进度', () => {
  it('扫描尚未返回就显示已发现数量，核对期间保留数量且不显示 0/0', async () => {
    const scan = deferred(), records = deferred();
    api.ScanDirectoryWithProgress.mockReturnValue(scan.promise);
    api.GetVideosByDirectory.mockReturnValue(records.promise);
    ...
    scan.resolve(['/share/videos/a.mp4']);
    await flushPromises();
    ...
    records.resolve([{ id: 1, path: '/share/videos/a.mp4' }]);
    await running;
  ...
  it('等待读取时显示经过时间，重开弹窗保留任务，忽略别的扫描事件', async () => {
    ...
    expect(api.ScanDirectoryWithProgress).toHaveBeenCalledOnce();
    scan.resolve([]);
    await running;
  ...
  it('进入入库阶段后显示真实总数并保持已有增删统计', async () => {
    const imported = deferred();
    api.ScanDirectoryWithProgress.mockResolvedValue(['/share/videos/new.mp4']);
    api.GetVideosByDirectory.mockResolvedValue([{ id: 7, path: '/share/videos/missing.mp4' }]);
    api.AddVideo.mockReturnValue(imported.promise);
    const { running } = await start();
    expect(wrapper.text()).toContain('正在处理 0/2');
    expect(wrapper.text()).toContain('共发现 1 个视频');
    imported.resolve();
    await running;
    expect(api.DeleteVideo).toHaveBeenCalledWith(7, false);
    expect(wrapper.text()).toContain('扫描完成：新增 1 个，删除 1 个');
  });
});
