      ...
      <section v-if="entityVideos.length" class="entity-video-grid" aria-label="相关视频">
        <article v-for="video in entityVideos" :key="video.id" class="entity-video-card glass-surface">
          ...
          <div class="entity-video-card__actions">
            <button type="button" class="btn-action btn-compact" @click="openVideoDetails(video)">预览</button>
...
.entity-video-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px; }.entity-video-card { min-width: 0; overflow: hidden; border-radius: 13px; }.entity-video-card__preview { width: 100%; aspect-ratio: 16 / 9; overflow: hidden; border: 0; background: #0f172a; cursor: pointer; }.entity-video-card__preview img { width: 100%; height: 100%; display: block; object-fit: cover; }.entity-video-card__copy { display: grid; gap: 4px; min-width: 0; padding: 11px 12px 7px; }.entity-video-card__copy strong,.entity-video-card__copy span,.entity-video-card__copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.entity-video-card__copy span,.entity-video-card__copy small { color: var(--text-muted); font-size: 11px; }.entity-video-card__actions { display: flex; justify-content: flex-end; gap: 7px; padding: 0 12px 11px; }
